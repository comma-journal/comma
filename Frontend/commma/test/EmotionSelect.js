...
// 감정 배열 (타입 제거)
const emotions = [
  // ... 기존 emotions 그대로, 각 객체에서 타입 선언 제거 ...
...
const EmotionSelect = () => {
  ...
  const cardHeight = 66;
  const totalGridWidth = cardWidth * 5 + 32;
  ...
  // 카드 호버 효과
  const handleCardHover = (emotionId, isHover) => {
  ...
  const handleEmotionSelect = (emotion) => {
    ...
    const currentY = selectedRow * 85 + 30;
